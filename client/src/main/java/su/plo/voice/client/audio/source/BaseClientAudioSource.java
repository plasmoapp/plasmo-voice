...
public abstract class BaseClientAudioSource<T extends SourceInfo> implements ClientAudioSource<T> {
    ...
    public BaseClientAudioSource(@NotNull PlasmoVoiceClient voiceClient,
    ...
    private void processAudioPacket(@NotNull SourceAudioPacket packet) {
        ...
        if (shouldCalculateOcclusion()) {
            ...
            double occlusion = calculateOcclusion(position);
            if (lastOcclusion >= 0) {
    ...
    private double calculateOcclusion(float[] position) {
