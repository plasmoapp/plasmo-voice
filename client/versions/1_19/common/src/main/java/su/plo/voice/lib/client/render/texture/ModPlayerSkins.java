package su.plo.voice.lib.client.render.texture;
...
public final class ModPlayerSkins implements MinecraftPlayerSkins {
    ...
    public synchronized CompletableFuture<String> loadSkin(@NotNull UUID playerId,
        ...
        RenderSystem.recordRenderCall(() -> {
            synchronized (this) {
                ...
                future.complete(skins.get(nick).toString());
            }
        });
    ...
    @Override
    public synchronized @NotNull String getSkin(@NotNull String nick) {
        return skins.getOrDefault(nick, steveSkin).toString();
    }
}
