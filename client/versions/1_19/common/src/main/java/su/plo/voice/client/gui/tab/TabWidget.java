...
public abstract class TabWidget extends ContainerObjectSelectionList<TabWidget.Entry> {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (Entry e : children()) {
            if (entry != e && e instanceof OptionEntry) {
                ...
                } else if (e.children().get(0) instanceof KeyBindingWidget) {
                    if (e.mouseClicked(mouseX, mouseY, button)) {
                        return true;
                    }
                }
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        ...
        for (Entry e : children()) {
            if (entry != e && e instanceof OptionEntry) {
                if (e.children().get(0) instanceof KeyBindingWidget) {
    ...
    protected void renderList(@NotNull PoseStack poseStack, int x, int y, int mouseX, int mouseY, float delta) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (l >= this.y0 && k <= this.y1) {
                ...
                r = this.getRowLeft();
                entry.render(poseStack, j, k, r, o, n, mouseX, mouseY, Objects.equals(this.hoveredEntry, entry), delta);
    ...
    public class OptionEntry<T extends AbstractWidget> extends Entry {
        ...
        public boolean mouseReleased(double mouseX, double mouseY, int button) {
            for (GuiEventListener entry : children()) {
                if (entry instanceof KeyBindingWidget && entry.mouseReleased(mouseX, mouseY, button)) {
                    return true;
                }
            }
