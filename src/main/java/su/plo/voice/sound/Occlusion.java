...
public class Occlusion {
    ...
    public static double getOccludedPercent(World world, ClientPlayerEntity player, Vector3d source) {
            ...
            if (!Double.isNaN(listener.x) && !Double.isNaN(listener.y) && !Double.isNaN(listener.z)) {
                ...
                while(i++ < 200) {
                    ...
                    if (i > 1) {
                        ...
                        if (!state.isAir() && !state.getShape(world, prevSoundPos).isEmpty() && collisionShape != VoxelShapes.empty()) {
                            BlockRayTraceResult rayTrace = collisionShape.clip(prevSound, listener, prevSoundPos);
                            if (rayTrace != null) {
