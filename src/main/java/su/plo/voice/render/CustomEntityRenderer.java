...
public class CustomEntityRenderer {
    ...
    public static void entityRender(PlayerEntity player, MatrixStack matrices, boolean hasLabel, IRenderTypeBuffer vertexConsumers, int light) {
        if(Voice.serverConfig == null) {
            renderIcon(Voice.SPEAKER_WARNING, player, matrices, hasLabel, vertexConsumers, light);
            return;
        }
        ...
        if(Voice.serverConfig.clients.contains(player.getUUID())) {
            if(Voice.clientMutedClients.contains(player.getUUID())) {
                renderIcon(Voice.SPEAKER_MUTED, player, matrices, hasLabel, vertexConsumers, light);
                ...
                if(SocketClientUDPQueue.talking.get(player.getUUID())) {
                    renderIcon(Voice.SPEAKER_PRIORITY, player, matrices, hasLabel, vertexConsumers, light);
                } else {
                    renderIcon(Voice.SPEAKER_ICON, player, matrices, hasLabel, vertexConsumers, light);
                }
            }
        } else {
            renderIcon(Voice.SPEAKER_WARNING, player, matrices, hasLabel, vertexConsumers, light);
    ...
    private static void renderIcon(ResourceLocation identifier, PlayerEntity player, MatrixStack matrices, boolean hasLabel,
                                   IRenderTypeBuffer vertexConsumers, int light) {
        matrices.pushPose();
        if(hasLabel) {
            matrices.translate(0D, player.getBbHeight() + 0.8D, 0D);
        } else {
            matrices.translate(0D, player.getBbHeight() + 0.5D, 0D);
        }
