...
public class VoiceServerConfig {
    ...
    public void update(ConfigPacket config) {
        ...
        if(Voice.config.servers.containsKey(ip)) {
            VoiceClientServerConfig serverConfig = Voice.config.servers.get(ip);
        ...
        Voice.recorder.updateConfig(config.getSampleRate());
